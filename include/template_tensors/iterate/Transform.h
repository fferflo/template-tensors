...
RETURN_AUTO(transform_iterator<typename std::decay<TFunctor&&>::type, TIterator>(iterator, util::forward<TFunctor>(functor)))
...
struct Transform
{
  ...
  RETURN_AUTO(::iterator::transform(util::forward<TIterator>(iterator), self.functor))
...
RETURN_AUTO(Transform<util::store_member_t<TFunctor&&>>(util::forward<TFunctor>(functor)))
...
RETURN_AUTO(
  adapt(util::forward<TIterable>(in), ::iterator::functor::transform(util::forward<TFunctor>(functor)))
)
