...
BOOST_AUTO_TEST_CASE(numpy_conversions)
{
  ...
  auto python = tt::boost::python::toNumpy(tensor1);
  auto tensor2 = tt::boost::python::fromNumpy<int, 3>(python, tensor1.template dims<3>());
...
BOOST_AUTO_TEST_CASE(numpy_moments)
{
  ...
  ::boost::python::numpy::ndarray python = tt::boost::python::toNumpy(tensor1);
  auto from_python = tt::boost::python::fromNumpy<float, 3>(python);
